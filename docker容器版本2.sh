...
port_status() {
  local port="$1"
  # Return codes:
  #   0 = sing-box 在该端口监听（TCP/UDP 任一）
  #   1 = 其他进程占用该端口（TCP 或 UDP）
  #   2 = 未监听/无法检测
  local have_tool=0
  local seen_singbox=0
  local seen_other=0
  ...
  if command -v ss >/dev/null 2>&1; then
    ...
    # TCP
    local out_t
    out_t=$(ss -ltnp "sport = :$port" 2>/dev/null || true)
    if grep -q LISTEN <<<"$out_t"; then
      if grep -Eqi 'users:\(\(".*sing-box' <<<"$out_t"; then
        seen_singbox=1
      else
        seen_other=1
      fi
    ...
    # UDP
    local out_u
    out_u=$(ss -lunp "sport = :$port" 2>/dev/null || true)
    if [[ -n "$out_u" ]] && ! grep -qE 'State|Recv-Q' <<<"$out_u"; then :; fi
    if grep -Eqi 'users:\(\(".*' <<<"$out_u"; then
      if grep -Eqi 'users:\(\(".*sing-box' <<<"$out_u"; then
        seen_singbox=1
      else
        seen_other=1
      fi
  ...
  if (( have_tool == 0 )) && command -v lsof >/dev/null 2>&1; then
    have_tool=1
    local names=""
    names+=$(lsof -nP -iTCP:"$port" -sTCP:LISTEN 2>/dev/null | awk 'NR>1{print $1}')
    names+=$(lsof -nP -iUDP:"$port" 2>/dev/null | awk 'NR>1{print $1}')
    if [[ -n "$names" ]]; then
      if echo "$names" | grep -Eqi 'sing-box'; then
        # 如果同时存在其他进程和 sing-box，优先认定 sing-box 占用（常见于多协议复用同端口）
        seen_singbox=1
      else
        seen_other=1
      fi
  ...
  if (( seen_singbox == 1 )); then
    return 0
  elif (( seen_other == 1 )); then
    return 1
  else
    return 2
  fi
...
ensure_rc_local_template() {
  # Create or normalize /etc/rc.local with a safe template
  local rc="/etc/rc.local"
  if [[ ! -f "$rc" ]]; then
    cat > "$rc" <<'RC'
#!/bin/sh -e
# rc.local generated by sing-box manager (autostart fallback)

# Wait a moment for network to settle
sleep 1

# Start sing-box via singleton wrapper to avoid double-starts
/usr/local/bin/sb-singleton >> /var/log/sing-box.log 2>&1 &

exit 0
RC
    chmod +x "$rc"
    echo "已创建标准化的 /etc/rc.local 模板（使用 sb-singleton）"
  else
    grep -q '^#!/bin/sh' "$rc" || sed -i '1i #!/bin/sh -e' "$rc"
    grep -q '^exit 0$' "$rc" || printf '\nexit 0\n' >> "$rc"
    # Ensure our start line exists exactly once, placed before exit 0
    ...
    chmod +x "$rc"
    echo "已标准化现有 /etc/rc.local 并注入 sb-singleton 自启"
  fi
}

install_singleton_wrapper() {
  # Create a tiny launcher that ensures only one sing-box instance is running.
  # It uses a pidfile and pgrep to ensure exclusivity.
...
# If pidfile exists and process alive, exit quietly
if [[ -f "$PIDFILE" ]]; then
...
# Also guard with pgrep exact match
...
WRAP
  ...
  echo "已安装 /usr/local/bin/sb-singleton（单例守护启动器）"
}

install_watchdog_cron() {
  # Lightweight watchdog: check every minute; if down, call the singleton wrapper.
  if ! command -v crontab >/dev/null 2>&1; then
    echo "未检测到 crontab，跳过 watchdog 安装"
    return 0
  fi
  local marker="# sing-box-watchdog"
  local line='* * * * * /usr/local/bin/sb-singleton >/dev/null 2>&1 ' + marker
  (crontab -l 2>/dev/null | grep -v "$marker"; echo "$line") | crontab -
  echo "已安装每分钟检查的 watchdog（使用 sb-singleton）"
}

install_autostart_fallback() {
  # Prefer rc.local only. Do NOT add @reboot if rc.local exists to avoid races.
  ensure_rc_local_template

  # If absolutely no rc.local support (rare), then @reboot as a last resort
  if [[ ! -f /etc/rc.local ]]; then
    if command -v crontab >/dev/null 2>&1; then
      ( crontab -l 2>/dev/null | grep -v '/usr/local/bin/sb-singleton' ; \
        echo '@reboot /usr/local/bin/sb-singleton' ) | crontab -
      echo "rc.local 不存在，已添加 @reboot 兜底（使用 sb-singleton）"
    else
      echo "rc.local 与 cron 均不可用，无法设置兜底自启"
    fi
  fi
}
...
install_logrotate() {
  # Prevent /var/log/sing-box.log from growing unbounded
...
LR
    echo "已安装 logrotate 规则：/etc/logrotate.d/sing-box"
  else
    echo "logrotate 规则已存在：/etc/logrotate.d/sing-box"
  fi
}
